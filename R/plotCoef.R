plotCoef=function(beta,norm,lambda,df,dev,bic,label=FALSE,xvar=c("norm","lambda","Index"),xlab=iname,ylab="Coefficients",...){
  ...
  switch(nwhich+1,#we add one to make switch work
         ...
         "1"=warning("1 or less nonzero coefficients; HotGeNew plot is not meaningful")
         )
...
# prettydf=ceiling(approx(x=index,y=df,xout=atdf,rule=2)$y)
 ...
 ind = min(which(bic == min(bic)));
 abline(v=index[ind],col='red');
}
